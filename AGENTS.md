...
2. `pnpm typecheck`
3. `pnpm test`
4. UI work: check it on the iOS simulator, not just web — SecureStore, auth,
...
- `ios/` is gitignored and regenerated by prebuild. Native changes go through
  `app.json` or a config plugin; never hand-edit `ios/` expecting it to be
...
- Session data and the private profile cache live in SecureStore. Drafts use
  `expo-sqlite` localStorage. Don't invent a new storage layer.
